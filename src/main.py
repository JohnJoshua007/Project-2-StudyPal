...
# Ensure device is not 'meta' – default to 'cpu' if not set or invalid
...
def setup_chain(selected_chapter, selected_subject):
    ...
    # 🔥 FIX: Use a safe device and explicitly set encode_kwargs
    embeddings = HuggingFaceEmbeddings(
        model_name="all-MiniLM-L6-v2",           # default model; change if needed
        model_kwargs={"device": DEVICE},         # will be "cpu" or "cuda"
        encode_kwargs={"device": DEVICE},        # important for encoding
    )
...
# Initialize chat and video history
if "chat_history" not in st.session_state:
    st.session_state.chat_history = []
if "video_history" not in st.session_state:
    st.session_state.video_history = []
